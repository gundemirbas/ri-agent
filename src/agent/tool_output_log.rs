...
///
/// # Directory selection
/// 1. `$XDG_RUNTIME_DIR/ri/tool-output/{session_id}/`  (Linux, when set)
/// 2. `{temp_dir}/ri-tool-output-{session_id}/`         (macOS, Windows, fallback)
...
fn resolve_dir(session_id: &str) -> PathBuf {
    ...
    if let Some(runtime) = std::env::var_os("XDG_RUNTIME_DIR") {
        ...
        if base.is_absolute() {
            return base.join("ri").join("tool-output").join(session_id);
        }
    }

    // Fallback: system temp dir.
    std::env::temp_dir().join(format!("ri-tool-output-{session_id}"))
}
